...
const STATIC_DATA_ENDPOINT = "./data/dashboard.json";
...
function renderDashboard(payload) {
  ...
  const normalizedHistory = normalizeHistory(snapshot, payload.history || []);
  ...
  updateText(dom.dbPathValue, meta.dataPath || meta.dbPath || "data/dashboard.json");
  ...
  const buildTime = meta.builtAt ? formatTimestamp(meta.builtAt) : formatTimestamp(snapshot.recordedAt);
  ...
  const chainText =
    snapshot.chainId === EXPECTED_CHAIN_ID
      ? `Static snapshot · built ${buildTime} · block ${formatInteger(snapshot.blockNumber)}${queueText}`
...
async function fetchStaticPayload() {
  return fetchJson(`${STATIC_DATA_ENDPOINT}?ts=${Date.now()}`, {
    method: "GET",
    cache: "no-store",
  });
}
